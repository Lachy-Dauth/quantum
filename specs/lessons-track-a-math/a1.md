...
- **Word count target:** Write as extensively as the material demands. Per CLAUDE.md, lessons are 8,000--25,000 words. A1 is the first lesson and must be thorough but welcoming. Do not target a specific count -- let the pedagogy dictate length.
- **Student time:** 120 -- 180 minutes (reading + working through examples + problem set across all parts)
- **Development time:** 16 -- 24 hours (writing + widget development + review)

---

### 12. Page Splits

A1 is split into **four parts**, each served at `/lessons/a1-complex-numbers/part-N`:

- **Part 1: The Argand Plane** — Motivation ("why complex numbers?"), the imaginary unit as a rotation, rectangular and polar form, conversion between forms. Definitions 3.1--3.2. Visual assets VA-1. Confusion callout CC-1. Historical notes on Cardano and the naming of "imaginary" numbers.

- **Part 2: Arithmetic as Geometry** — Addition as vector addition, multiplication as rotate-and-scale, division via the conjugate, conjugate identities, modulus multiplicativity, the triangle inequality. Lemma 3.3, Proposition 3.4, Theorem 3.5. Visual assets VA-2, VA-6. Worked Examples 5.1, 5.4. Confusion callouts CC-2, CC-5.

- **Part 3: Euler's Formula and Roots of Unity** — n-th roots of unity and regular polygons, the sum-to-zero property, Euler's formula via Taylor series and the ODE argument, Euler's identity, key angle values. Proposition 3.8, Lemma 3.9, Theorem 3.6, Corollary 3.7. Visual assets VA-3, VA-4. Worked Examples 5.2, 5.3, 5.5. Confusion callout CC-3.

- **Part 4: Phase in Quantum Mechanics** — States as complex vectors, global phase, relative phase, why interference makes phase matter. Definition 3.10. Visual asset VA-5. Confusion callout CC-4. Full problem set (A1.1--A1.10) with collapsible solutions.

Rationale: Four parts at natural conceptual boundaries prevent any single page from feeling overwhelming. Each part is self-contained enough to be a satisfying reading session. The problem set is consolidated in Part 4 so students encounter it after the full theoretical arc.
