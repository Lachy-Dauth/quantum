...
Now compute the QM predictions for the singlet state: E(a,b) = -a . b = -cos(theta\_{ab}).

Choose a, b, c coplanar with angles: theta*{ab} = pi/3, theta*{ac} = 2pi/3, theta\_{bc} = pi/3.
...
- Expand a . sigma tensor b . sigma = sum\_{ij} a_i b_j (sigma_i tensor sigma_j).
- Using the result <Psi^-| sigma*i tensor sigma_j |Psi^-> = -delta*{ij} (derived from the singlet's rotational invariance or by direct computation for each i,j):
...
_Problem:_ Show that |Psi^-> = (|01> - |10>)/sqrt(2) is entangled (cannot be written as a product state).
...
_Problem:_ For the singlet state |Psi^->, compute E(z, z), E(z, x), and E(z, n_hat) where n_hat = (sin theta, 0, cos theta).
...
_Problem:_ For the singlet state, compute the CHSH parameter S with the settings: a = 0 (z-axis), a' = pi/2 (x-axis), b = pi/4 (45 degrees), b' = -pi/4 (-45 degrees), where angles are measured from the z-axis in the xz-plane.
...
_Problem:_ Alice and Bob share the singlet state |Psi^->. Alice measures S_z. What is Bob's reduced density matrix before and after Alice's measurement? Show they are identical.
...
_Problem:_ Compute the CHSH parameter S for the product state |00> with the same optimal settings as WE-P6.3.

_Solution:_
For |00> = |0>\_A tensor |0>\_B:
E(a, b) = <00| (a . sigma tensor b . sigma) |00> = <0|a . sigma|0> _ <0|b . sigma|0> = a_z _ b*z = cos(0) * cos(pi/4) = 1 \_ sqrt(2)/2 = sqrt(2)/2.

E(a, b') = a*z * b'\_z = cos(0) _ cos(-pi/4) = sqrt(2)/2.
E(a', b) = a'\_z _ b*z = cos(pi/2) * cos(pi/4) = 0.
E(a', b') = a'\_z \* b'\_z = 0.
