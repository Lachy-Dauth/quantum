  ...
  return (
    <div className="flex flex-col lg:flex-row" style={{ minHeight: height }}>
      ...
      >
        ...
        {/* Axis labels */}
        <text x={W - 16} y={oy - 8} fontSize={13} className="fill-slate-500 dark:fill-slate-400" textAnchor="end">Re</text>
        <text x={ox + 8} y={16} fontSize={13} className="fill-slate-500 dark:fill-slate-400">Im</text>
        ...
        {Array.from({ length: Math.floor(RANGE * 2) + 1 }, (_, i) => {
          ...
          return (
            <g key={`tick-${v}`}>
              <text x={sx} y={oy + 16} fontSize={10} textAnchor="middle" className="fill-slate-400 dark:fill-slate-500">{v}</text>
              <text x={ox - 8} y={sy + 4} fontSize={10} textAnchor="end" className="fill-slate-400 dark:fill-slate-500">{v}i</text>
            </g>
          )
        })}
        ...
        <line x1={zx} y1={zy} x2={zx} y2={oy} stroke="#3b82f6" strokeWidth={1} strokeDasharray="4 3" opacity={0.5} />
        <line x1={zx} y1={zy} x2={ox} y2={zy} stroke="#3b82f6" strokeWidth={1} strokeDasharray="4 3" opacity={0.5} />
        ...
        {mod > 0.1 && (
          <polygon
            points={arrowHead(ox, oy, zx, zy, 8)}
            fill="#3b82f6"
          />
        )}

        {/* Point z */}
        <circle cx={zx} cy={zy} r={6} fill="#3b82f6" stroke="white" strokeWidth={2} className="cursor-grab" />
      ...
      <div className="w-full lg:w-52 p-4 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-sm space-y-3">
        <div>
          <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Rectangular</div>
          <div className="font-mono text-slate-800 dark:text-slate-200">{fmtComplex(z)}</div>
        </div>
        <div>
          <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Polar</div>
          <div className="font-mono text-slate-800 dark:text-slate-200">{fmtNum(mod)}(cos {fmtAngle(arg)} + i sin {fmtAngle(arg)})</div>
        </div>
        <div>
          <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Exponential</div>
          <div className="font-mono text-slate-800 dark:text-slate-200">{fmtNum(mod)}e<sup>i{fmtAngle(arg)}</sup></div>
        </div>
        <hr className="border-slate-200 dark:border-slate-700" />
        <div className="space-y-1">
          <div className="flex justify-between"><span className="text-slate-500">|z|</span><span className="font-mono">{fmtNum(mod, 3)}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">arg(z)</span><span className="font-mono">{fmtAngle(arg)}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Re(z)</span><span className="font-mono">{fmtNum(z[0], 3)}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Im(z)</span><span className="font-mono">{fmtNum(z[1], 3)}</span></div>
        </div>
        <hr className="border-slate-200 dark:border-slate-700" />
